...
id=$(sbatch $dep --ntasks=1 --cpus-per-task=1 -N 1 --job-name=${name}.genLag --output=${name}_genLag.out --mem=${mem}000 $queue --time=${days}-0 --wrap="python3 genLag.py -r $reps -n $name -l $loci")
echo $id
id=${id##* }
dep="--dependency=afterok:$id"

id=$(sbatch $dep --ntasks=1 --cpus-per-task=1 -N 1 --job-name=${name}.genoTime --output=${name}_genoTime.out --mem=${mem}00 $queue --time=${days}-0 --wrap="python3 genoTime.py -n $name -le $eloci -l $loci -r $reps")
echo $id
id=${id##* }
dep="--dependency=afterok:$id"

 id=$(sbatch $dep --ntasks=1 --cpus-per-task=1 -N 1 --job-name=${name}.poptime --output=${name}_poptime.out --mem=${mem}00 $queue --time=${days}-0 --wrap="python3 poptime.py -l $i -s $p -d $k")
echo $id
id=${id##* }
dep="--dependency=afterok:$id"
